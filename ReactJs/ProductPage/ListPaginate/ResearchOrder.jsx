...
const ResearchOrder = ({
  ...
  function handleEditSubmit(updatedData) {
    const updatedRow = data.id === updatedData.id;
      ...
      {data ? (
        <>
          ...
          <Table>
            ...
            <TableBody>
              {data && (
                <TableRow key={data?.id} role="checkbox" tabIndex={-1}>
                  <TableCell padding="checkbox">
                    <Checkbox
                  ...
                  {columns.map((column) => (
                    <TableCell
                      className={column.clasess}
                      key={`${data?.id}-${column?.name}`}
                      align={column?.align}
                      onClick={() => handleEditClick(data, column)}
                    >
                      {column.cast
                        ? column.cast(data[column.field])
                        : data[column?.field]}
                    </TableCell>
                  ))}
                </TableRow>
...
export default memo(ResearchOrder);
